...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, self.pullToRefreshView.frame.size.height);
        }
    }
}
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGFloat margin = 10;
        CGFloat marginY = 2;
        CGFloat marginTop = self.topNothing ? 12 : 0;
        ...
        CGSize titleSize = [Helper sizeForLabelWithString:self.titleLabel.text font:self.titleLabel.font constrainedToSize:CGSizeMake(labelMaxWidth, self.titleLabel.font.lineHeight)];
        
        
        CGSize subtitleSize = [Helper sizeForLabelWithString:self.subtitleLabel.text font:self.subtitleLabel.font constrainedToSize:CGSizeMake(labelMaxWidth, self.subtitleLabel.font.lineHeight)];
        
        CGFloat maxLabelWidth = MAX(ceilf(titleSize.width),ceilf(subtitleSize.width));
        ...
        if(subtitleSize.height > 0){
            CGFloat totalHeight = titleSize.height + subtitleSize.height + marginY + marginTop;
            ...
            CGFloat minY = (self.bounds.size.height / 2)  - (totalHeight / 2) + marginTop;
        ...
        self.arrow.frame = CGRectMake(arrowX,
                                      (self.bounds.size.height / 2) - (self.arrow.bounds.size.height / 2) + marginTop,
...
- (void)drawRect:(CGRect)rect {
    ...
    //    CGContextAddRect(c, CGRectMake(10, 0, 2, 46));
    // the arrow
    CGContextMoveToPoint(c, 5, 28);
    CGContextAddLineToPoint(c, 11, 35);
    CGContextAddLineToPoint(c, 17, 28);
    //	CGContextAddLineToPoint(c, 0, 34);
    CGContextSetLineWidth(c, 1);
    CGContextSetStrokeColorWithColor(c, UIColorFromRGB(0xA0A0A0).CGColor);
