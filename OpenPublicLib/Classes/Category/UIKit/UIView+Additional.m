...
- (void)createTopLineWithBackgroundColor:(UIColor *)color {
    if(self.height <= 0) {
        return;
    }
    NSInteger tag = NSIntegerMax - 1;
    UIView* topLineView = [self viewWithTag:tag];
    if (!topLineView) {
    ...
    topLineView.frame = CGRectMake(0, 0, self.width, kOnePixelWidth);
}


- (void)createBottomLineWithBackgroundColor:(UIColor *)color {
    if(self.height <= 0) {
        return;
    }
    NSInteger tag = NSIntegerMax - 2;
    UIView* bottomLineView = [self viewWithTag:tag];
    if (!bottomLineView) {
    ...
    bottomLineView.frame = CGRectMake(0, self.height - kOnePixelWidth, self.width, kOnePixelWidth);
}
