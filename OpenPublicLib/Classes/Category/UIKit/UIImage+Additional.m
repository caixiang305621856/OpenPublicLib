...
+ (UIImage *)imageWithColors:(CFArrayRef)colors size:(CGSize)size
{
    @autoreleasepool {
        CGRect rect = CGRectMake(0, 0, size.width, size.height);
        UIGraphicsBeginImageContext(rect.size);
        CGContextRef ctx = UIGraphicsGetCurrentContext();
        CGGradientRef gradientRef = CGGradientCreateWithColors(CGColorSpaceCreateDeviceRGB(), (CFArrayRef)colors,  (CGFloat[]){
            0.0f,       // 对应起点颜色位置
            1.0f // 对应终点颜色位置
        });
